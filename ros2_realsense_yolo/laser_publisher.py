...
class SimpleObjectPublisherClass(Node):

    def __init__(self):
        self.model = YOLO("yolov8n-pose.pt")

        super().__init__('simple_object_dectation')
        ...
        for s in self.device.sensors:
            if s.get_info(rs.camera_info.name) == 'RGB Camera':
                self.found_rgb = True
                self.arrayPublisher_ = self.create_publisher(PoseArray, 'object', 10)
    ...
    def loop(self):
        ...
        self.layoutArray = []
        self.positionArray=[]
        laser_scan_data = []    #スキャンデータの全データ

        self.laserHeight = 0.2 #画素上のスキャン高さ 0.25:胴 0.5:モモ 

        for x in range(self.w):
            pixelValue = self.verts[x + int(self.h * self.laserHeight) * self.w]            
            laser_scan_data.append(pixelValue)

        for lsd in laser_scan_data:
            self.poses = Pose()
...
def main(args=None):
    ...
    publisher = SimpleObjectPublisherClass()
